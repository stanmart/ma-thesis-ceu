...
from scipy.interpolate import PchipInterpolator,PchipInterpolator
...
def get_policy_functions(r, params):
    ...
    while diffC > 1e-8 and itr < 2000:
        ...
        for i in es:
            cSplines.append(PchipInterpolator(mGrid[:,i], c[:,i]))
        ...
        for i in es:
            spl = PchipInterpolator(mGridNew[:,i], cNew[:,i])
...
def excess_asset_demand(params, grids, shocks, return_distr=False):
    ...
    sSplines = []
    for i in es:
        sSplines.append(PchipInterpolator(sGrid[:,i], aGrid))
...
def paramfun3(paramlist):
    try:
        return get_eq_r_het(paramlist, SHOCKS)
    except:
        return np.array([np.nan])
